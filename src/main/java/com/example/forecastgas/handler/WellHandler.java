...
public class WellHandler implements Handler{

    private int kol_skvajina;
...
//        System.out.println("P_p = "+P_p);
        ...
        double sredniy_debit_skvajina=(reserves.Q_const()/(forecast.kol_den*kol_skvajina))*1000;
...
//        if (P_u<=P_u_limit) return iteration_production_decrease_P_u_constant(inYearLimit, reserves, numberOfWells, y,  day,  limit,P_u_limit);
    ...
    public boolean check(Forecast forecast, GasReserves reserves) {
        ...
        if (new Double(P_zaboy).isNaN()){
            for (int i = forecast.kol_skvajina+1; i <=reserves.well_const() ; i++) {
                sredniy_debit_skvajina=(reserves.Q_const()/(forecast.kol_den*i))*1000;
